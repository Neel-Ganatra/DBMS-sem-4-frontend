...
const Player_role = () => {
  ...
  const [player_role, setPlayer_role] = useState([]);

  const deletePlayer_role = (player_roleId) => {
    const confirmDelete = window.confirm(
      "Are you sure you want to delete this player_role?"
    );
    if (!confirmDelete) {
      return;
    }
    axios
      .delete(`/admin/player_role/${player_roleId}`) // Use proper string interpolation
      .then((res) => {
          ...
          >
            <div className="three-ones">
              <div className="Employee_search_divv">
                <span className="Order-history-span">ROLES</span>
              ...
              <Link to={ROUTES.ADD_ROLE} className="btn btn-success">
                Add ROLE
              </Link>
            </div>
          </div>
          ...
          >
            ...
            <tbody>
              {player_role.map((player_role) => (
                <tr key={player_role.id}>
                  <td>{player_role.roles}</td>
                  <td>
                    <div className="d-flex gap-2 w-100">
                      ...
                      <button
                        ...
                        onClick={() => deletePlayer_role(player_role.id)}
                      >
                        <img src={remove} alt="" />
                      </button>
                      <button
                        ...
                        onClick={() =>
                          navigate(ROUTES.EDIT_ROLE + player_role.id)
                        }
                      >
                        <img src={edit} alt="" />
                      </button>
